      ...
      <div className="flex-grow flex flex-col p-4 md:p-6 lg:p-8 overflow-hidden max-w-4xl mx-auto w-full">
        <div className="flex items-center mb-4 bg-white rounded-lg shadow p-3">
          <img
            ...
            className="w-12 h-12 object-cover rounded-full mr-3 border-2 border-indigo-300"
          />
          ...
          {messages.length === 0 ? (
            ...
            messages.map((msg) => (
              <div
                key={msg.id}
                className={`mb-3 ${
                  msg.senderId === authUser!.id ? "text-right" : "text-left"
                }`}
              >
                <span
...
const MatchNotFound = () => (
  <div className="h-screen flex flex-col items-center justify-center bg-gray-100 bg-opacity-50 bg-dot-pattern">
    <div className="bg-white p-8 rounded-lg shadow-md text-center">
      <UserX size={64} className="mx-auto text-indigo-500 mb-4" />
      ...
      <Link
        to="/"
        className="mt-6 px-4 py-2 bg-indigo-500 text-white rounded hover:bg-indigo-600 transition-colors 
				focus:outline-none focus:ring-2 focus:ring-indigo-300 inline-block"
      >
        Go Back To Home
      </Link>
    </div>
